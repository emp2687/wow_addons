...
function EasyLoot_HandleRoll(id)
	...
	local _, _, _, iLevel,  _, _, _, _,  _, _, _ = GetItemInfo(link)
  	...
  	if(EasyLoot_InTable(EasyLootLootList.need, name) and canNeed) then
  		...
  		RollOnLoot(id, 1)
		return
	elseif(EasyLoot_InTable(EasyLootLootList.ignore, name)) then
	  	--DEFAULT_CHAT_FRAME:AddMessage("|cffffff00EasyLoot item up for rolling in ignore: "..name)
  		RollOnLoot(id, 0)
		return
...
function EasyLoot_InTable2(t, val)
	for i=1, #(t), 1 do
		...
		if(type(t[i]) == "table") then
  		...
  		if strmatch(val, localValue) then
		    return i
  		end
	end
	return false
