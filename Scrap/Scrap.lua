...
function Scrap:IsSellItem(name, itemQuality, itemLevel, itemMinLevel, itemType)
	for _, v in pairs(sellExceptions) do
		if v == name then
			return false
		end
	end
	...
	if itemType == 'Armor' or itemType == 'Weapon' then
		--[[
		if itemQuality == ITEM_QUALITY_UNCOMMON and (itemMinLevel >= 20 or itemLevel >= 30) then
			return true
		end
		]]--
		if itemQuality == ITEM_QUALITY_COMMON then
			return true
		end
	end
...
function Scrap:IsJunk(itemID)
	if itemID then
		local name, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, _, itemSlot, _, itemValue = GetItemInfo(itemID)
		local valuable = itemQuality ~= ITEM_QUALITY_POOR or itemValue == 0
		
		if Scrap:IsSellItem(name, itemQuality, itemLevel, itemMinLevel, itemType) then
			return true
		end
